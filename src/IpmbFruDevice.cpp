...
void findIpmbDev(std::vector<uint8_t>& ipmbAddr)
{
    ...
    for(int i=0; i<ipmbAddr.size(); i++)
    {
        printf("Ipmb Dev Index.. : %d\n ", ipmbAddr.at(i));
    }

} 


void getAreaInfo(uint8_t ipmbAddress, uint8_t &compCode)
{
    uint8_t cmd = 0x10;   
    uint8_t netFn = 0xa; 
...
{ 
    ...
    std::cout << "Ipmb resp data - Read FRU : ";
    for(int i=0; i<data.size(); i++)
    {
       printf("%d :", data.at(i));
       std::cout.flush();
...
{ 
    ...
    auto devMap = std::make_shared<DeviceMap>();
    ...
    for(iter=0; iter<ipmbAddress.size(); iter++)
    {
       ...
       getAreaInfo(ipmbAddress.at(iter), compCode);
       ...
       devMap->emplace(count, fruResponse); 
       for(auto& device : *devMap)
       {   
           AddFruObjectToDbus(device.second, dbusInterfaceMap, iter, device.first);
       }
       devMap->clear();
    }
}
